...
export default function Register() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (phoneDigits && selectedCountry && phoneDigits.length !== selectedCountry.phoneDigits) {
      setError(`Le numéro de téléphone doit comporter ${selectedCountry.phoneDigits} chiffres pour ${form.country}.`)
      return
    }
    ...
    try {
      await register({
        ...form,
        phone: phoneDigits && selectedCountry ? `+${selectedCountry.dialCode}${phoneDigits}` : undefined,
      })
      navigate('/')
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <PhoneCountryInput
            ...
            onCountryChange={(name) => setForm((f) => ({ ...f, country: name }))}
