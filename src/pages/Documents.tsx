...
export default function Documents() {
  ...
  const load = async () => {
    ...
    try {
      setItems(await fetchDocuments(organizationId))
    } catch (e: any) {
      setError(
  ...
  const handleUpload = async (file?: File) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError(
        `Le fichier « ${file.name} » est trop volumineux. La taille maximale autorisée est de 20 Mo.`,
      )
    ...
    if (!ACCEPTED_EXTENSIONS.includes(extension)) {
      setError(
        `Le format « ${extension || 'inconnu'} » n'est pas accepté. Formats autorisés : PDF, JPG, JPEG, PNG, DOC, DOCX, XLS et XLSX.`,
      )
    ...
    // Vérification du type MIME lorsque le navigateur le fournit
    if (file.type && !ACCEPTED_MIME_TYPES.includes(file.type)) {
      setError(
        `Le type de fichier « ${file.type} » n'est pas accepté. Formats autorisés : PDF, JPG, JPEG, PNG, DOC, DOCX, XLS et XLSX.`,
      )
  ...
  const remove = async (item: DocumentItem) => {
    if (
      !organizationId ||
      !window.confirm(`Supprimer « ${item.original_name} » ?`)
    ) {
      return
    }

    try {
      await deleteDocument(organizationId, item.id)
  ...
  if (!organizationId) {
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="px-4 pt-24 text-slate-600 lg:ml-[var(--finance-sidebar-width)] lg:px-8">
        ...
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Centralisez les factures, reçus, contrats et autres
                justificatifs de l’organisation.
              </p>

              <p className="mt-2 text-xs text-slate-400">
                Formats acceptés : PDF, JPG, JPEG, PNG, DOC, DOCX, XLS,
                XLSX — Taille maximale : 20 Mo par fichier.
              </p>
            </div>
            ...
            >
              ...
              {uploading ? 'Import...' : 'Ajouter une pièce'}
            </button>
          ...
          <section className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900">
                {formatSize(
                  items.reduce((sum, x) => sum + x.size, 0),
                )}
              </p>
            </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-100 p-4 md:flex-row md:items-center md:justify-between">
              ...
              <button
                onClick={load}
                disabled={loading}
                className="inline-flex items-center gap-2 self-end rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 disabled:opacity-50"
              >
                <RefreshCw
                  size={15}
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-5">
            <label className="text-sm font-semibold text-slate-700">
              Description du prochain document
            </label>

            <input
              value={description}
              onChange={e => setDescription(e.target.value)}
              placeholder="Ex. Facture fournisseur janvier 2026"
              className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400"
            />

            <p className="mt-2 text-xs text-slate-400">
              Les fichiers doivent être au format PDF, JPG, JPEG, PNG,
              DOC, DOCX, XLS ou XLSX et ne doivent pas dépasser 20 Mo.
            </p>
          </div>
